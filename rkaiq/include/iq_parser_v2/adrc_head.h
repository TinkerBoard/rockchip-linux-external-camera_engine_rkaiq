...
typedef struct MotionData_s {
    ...
    float* MotionCoef;
    int MotionCoef_len;
    // M4_ARRAY_DESC("MotionStr", "f32", M4_SIZE(1,100), M4_RANGE(0,1), "0",M4_DIGIT(3),
    // M4_DYNAMIC(1))
...
typedef struct HighLightDataV12_s {
    ...
    float* Strength;  // sw_drc_weig_maxl,  range[0,1], step 1/16
    int Strength_len;
    // M4_ARRAY_DESC("gas_t", "f32", M4_SIZE(1,100), M4_RANGE(0,4), "1",M4_DIGIT(3), M4_DYNAMIC(1))
    float* gas_t;  // sw_adrc_gas_t,  range[0,1], step 1/16
    int gas_t_len;
} HighLightDataV12_t;
...
typedef struct CalibDbV2_Adrc_V12_s {
    ...
    // "[0,2,20,76,193,381,631,772,919,1066,1211,1479,1700,1863,1968,2024,2048]", M4_DIGIT(0),
    // M4_DYNAMIC(0))
    int Scale_y[ADRC_Y_NUM];
    // M4_NUMBER_DESC("ByPassThr", "f32", M4_RANGE(0,1), "0", M4_DIGIT(4))
    float ByPassThr;
