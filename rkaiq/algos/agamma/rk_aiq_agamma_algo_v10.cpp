...
void AgammaProcessing(AgammaHandle_t* pGammaCtx) {
    ...
    if (pGammaCtx->agammaAttrV10.mode == RK_AIQ_GAMMA_MODE_AUTO) {  // run iq gamma
    ...
    } else if (pGammaCtx->agammaAttrV10.mode ==
...
void AgammaSetProcRes(AgammaProcRes_t* AgammaProcRes, rk_aiq_gamma_cfg_t* agamma_config) {
    ...
    AgammaProcRes->Gamma_v10.gamma_en = agamma_config->gamma_en;
    if (AgammaProcRes->Gamma_v10.gamma_en) {
        AgammaProcRes->Gamma_v10.equ_segm = agamma_config->gamma_out_segnum;
        AgammaProcRes->Gamma_v10.offset   = agamma_config->gamma_out_offset;
        for (int i = 0; i < CALIBDB_AGAMMA_KNOTS_NUM_V10; i++)
            AgammaProcRes->Gamma_v10.gamma_y[i] = agamma_config->gamma_table[i];
    }
