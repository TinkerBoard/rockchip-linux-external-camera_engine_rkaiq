...
#include "rk_aiq_agamma_algo_v1.h"
...
XCamReturn AgammaInit(AgammaHandle_t** pGammaCtx, CamCalibDbV2Context_t* pCalib)
{
    ...
    memcpy(&handle->CalibDb, calibv2_agamma_calib, sizeof(CalibDbV2_gamma_t));
...
void AgammaAutoProc(AgammaHandle_t* pGammaCtx)
{
    ...
    pGammaCtx->agamma_config.gamma_out_segnum = pGammaCtx->CalibDb.GammaTuningPara.Gamma_out_segnum;
    pGammaCtx->agamma_config.gamma_out_offset = pGammaCtx->CalibDb.GammaTuningPara.Gamma_out_offset;

    for(int i = 0; i < CALIBDB_AGAMMA_KNOTS_NUM; i++) {
        int tmp = pGammaCtx->CalibDb.GammaTuningPara.Gamma_curve[i];
...
void AgammaProcessing(AgammaHandle_t* pGammaCtx)
{
    ...
    if(ApiMode == RK_AIQ_GAMMA_MODE_OFF) { //run iq gamma
        ...
        pGammaCtx->agamma_config.gamma_en = pGammaCtx->CalibDb.GammaTuningPara.Gamma_en;
