...
(
    ...
    pAmergeCtx->CurrData.HandleData.Merge_v20.OECurve_smooth = GetCurrPara(pAmergeCtx->CurrData.CtrlData.EnvLv,
            pAmergeCtx->ConfigV1.EnvLv, pAmergeCtx->ConfigV1.OECurve_smooth, pAmergeCtx->ConfigV1.MaxEnvLvKnots);
    pAmergeCtx->CurrData.HandleData.Merge_v20.OECurve_offset = GetCurrPara(pAmergeCtx->CurrData.CtrlData.EnvLv,
            pAmergeCtx->ConfigV1.EnvLv, pAmergeCtx->ConfigV1.OECurve_offset, pAmergeCtx->ConfigV1.MaxEnvLvKnots);
    ...
    pAmergeCtx->CurrData.HandleData.Merge_v20.MDCurveLM_smooth = GetCurrPara(pAmergeCtx->CurrData.CtrlData.MoveCoef,
            pAmergeCtx->ConfigV1.MoveCoef, pAmergeCtx->ConfigV1.MDCurveLM_smooth, pAmergeCtx->ConfigV1.MaxMoveCoefKnots);
    pAmergeCtx->CurrData.HandleData.Merge_v20.MDCurveLM_offset = GetCurrPara(pAmergeCtx->CurrData.CtrlData.MoveCoef,
            pAmergeCtx->ConfigV1.MoveCoef, pAmergeCtx->ConfigV1.MDCurveLM_offset, pAmergeCtx->ConfigV1.MaxMoveCoefKnots);
    pAmergeCtx->CurrData.HandleData.Merge_v20.MDCurveMS_smooth = GetCurrPara(pAmergeCtx->CurrData.CtrlData.MoveCoef,
            pAmergeCtx->ConfigV1.MoveCoef, pAmergeCtx->ConfigV1.MDCurveMS_smooth, pAmergeCtx->ConfigV1.MaxMoveCoefKnots);
    pAmergeCtx->CurrData.HandleData.Merge_v20.MDCurveMS_offset = GetCurrPara(pAmergeCtx->CurrData.CtrlData.MoveCoef,
            pAmergeCtx->ConfigV1.MoveCoef, pAmergeCtx->ConfigV1.MDCurveMS_offset, pAmergeCtx->ConfigV1.MaxMoveCoefKnots);

    pAmergeCtx->CurrData.CtrlData.MergeOEDamp = pAmergeCtx->ConfigV1.OECurve_damp;
    pAmergeCtx->CurrData.CtrlData.MergeMDDampLM = pAmergeCtx->ConfigV1.MDCurveLM_damp;
    pAmergeCtx->CurrData.CtrlData.MergeMDDampMS = pAmergeCtx->ConfigV1.MDCurveMS_damp;
...
(
    ...
    pAmergeCtx->ConfigV1.BaseFrm = BASEFRAME_LONG;
    pAmergeCtx->ConfigV1.ByPassThr = LIMIT_VALUE(pAmergeCtx->CalibDBV1.MergeTuningPara.ByPassThr, DAMPMAX, DAMPMIN);
    pAmergeCtx->ConfigV1.OECurve_damp = LIMIT_VALUE(pAmergeCtx->CalibDBV1.MergeTuningPara.OECurve_damp, DAMPMAX, DAMPMIN);
    pAmergeCtx->ConfigV1.MDCurveLM_damp = LIMIT_VALUE(pAmergeCtx->CalibDBV1.MergeTuningPara.MDCurveLM_damp, DAMPMAX, DAMPMIN);
    pAmergeCtx->ConfigV1.MDCurveMS_damp = LIMIT_VALUE(pAmergeCtx->CalibDBV1.MergeTuningPara.MDCurveMS_damp, DAMPMAX, DAMPMIN);
    for (int i = 0; i < pAmergeCtx->ConfigV1.MaxEnvLvKnots; i++ ) {
        pAmergeCtx->ConfigV1.EnvLv[i] = LIMIT_VALUE(pAmergeCtx->CalibDBV1.MergeTuningPara.OECurve.EnvLv[i], ENVLVMAX, ENVLVMIN);
        pAmergeCtx->ConfigV1.OECurve_smooth[i] = LIMIT_VALUE(pAmergeCtx->CalibDBV1.MergeTuningPara.OECurve.Smooth[i], IQPARAMAX, IQPARAMIN);
        pAmergeCtx->ConfigV1.OECurve_offset[i] = LIMIT_VALUE(pAmergeCtx->CalibDBV1.MergeTuningPara.OECurve.Offset[i], OECURVEOFFSETMAX, OECURVEOFFSETMIN);
    }
    for (int i = 0; i < pAmergeCtx->ConfigV1.MaxMoveCoefKnots; i++ ) {
        pAmergeCtx->ConfigV1.MoveCoef[i] = LIMIT_VALUE(pAmergeCtx->CalibDBV1.MergeTuningPara.MDCurve.MoveCoef[i], MOVECOEFMAX, MOVECOEFMIN);
        pAmergeCtx->ConfigV1.MDCurveLM_smooth[i] = LIMIT_VALUE(pAmergeCtx->CalibDBV1.MergeTuningPara.MDCurve.LM_smooth[i], IQPARAMAX, IQPARAMIN);
        pAmergeCtx->ConfigV1.MDCurveLM_offset[i] = LIMIT_VALUE(pAmergeCtx->CalibDBV1.MergeTuningPara.MDCurve.LM_offset[i], IQPARAMAX, IQPARAMIN);
        pAmergeCtx->ConfigV1.MDCurveMS_smooth[i] = LIMIT_VALUE(pAmergeCtx->CalibDBV1.MergeTuningPara.MDCurve.MS_smooth[i], IQPARAMAX, IQPARAMIN);
        pAmergeCtx->ConfigV1.MDCurveMS_offset[i] = LIMIT_VALUE(pAmergeCtx->CalibDBV1.MergeTuningPara.MDCurve.MS_offset[i], IQPARAMAX, IQPARAMIN);
    }

    //turn the IQ paras into algo paras
    for(int i = 0; i < pAmergeCtx->ConfigV1.MaxEnvLvKnots; i++)
    {
        pAmergeCtx->ConfigV1.OECurve_smooth[i] *= OECURVESMOOTHMAX;
        pAmergeCtx->ConfigV1.OECurve_smooth[i] = LIMIT_VALUE(pAmergeCtx->ConfigV1.OECurve_smooth[i], OECURVESMOOTHMAX, OECURVESMOOTHMIN) ;
    }

    for(int i = 0; i < pAmergeCtx->ConfigV1.MaxMoveCoefKnots; i++)
    {
        pAmergeCtx->ConfigV1.MDCurveLM_smooth[i] *= MDCURVESMOOTHMAX;
        pAmergeCtx->ConfigV1.MDCurveLM_smooth[i] = LIMIT_VALUE(pAmergeCtx->ConfigV1.MDCurveLM_smooth[i], MDCURVESMOOTHMAX, MDCURVESMOOTHMIN) ;
        pAmergeCtx->ConfigV1.MDCurveLM_offset[i] *= MDCURVEOFFSETMAX;
        pAmergeCtx->ConfigV1.MDCurveLM_offset[i] = LIMIT_VALUE(pAmergeCtx->ConfigV1.MDCurveLM_offset[i], MDCURVEOFFSETMAX, MDCURVEOFFSETMIN) ;

        pAmergeCtx->ConfigV1.MDCurveMS_smooth[i] *= MDCURVESMOOTHMAX;
        pAmergeCtx->ConfigV1.MDCurveMS_smooth[i] = LIMIT_VALUE(pAmergeCtx->ConfigV1.MDCurveMS_smooth[i], MDCURVESMOOTHMAX, MDCURVESMOOTHMIN) ;
        pAmergeCtx->ConfigV1.MDCurveMS_offset[i] *= MDCURVEOFFSETMAX;
        pAmergeCtx->ConfigV1.MDCurveMS_offset[i] = LIMIT_VALUE(pAmergeCtx->ConfigV1.MDCurveMS_offset[i], MDCURVEOFFSETMAX, MDCURVEOFFSETMIN) ;
    }
...
(
    ...
    int New_OECurve_EnvLv_len = pAmergeCtx->CalibDBV1.MergeTuningPara.OECurve.EnvLv_len;
    int New_MDCurve_MoveCoef_len = pAmergeCtx->CalibDBV1.MergeTuningPara.MDCurve.MoveCoef_len;

    LOG1_AMERGE( "%s: Pre MaxEnvLvKnots:%d Cur MaxEnvLvKnots:%d\n", __FUNCTION__, pAmergeCtx->ConfigV1.MaxEnvLvKnots, New_OECurve_EnvLv_len);
    LOG1_AMERGE( "%s: Pre MaxMoveCoefKnots:%d Cur MaxMoveCoefKnots:%d\n", __FUNCTION__, pAmergeCtx->ConfigV1.MaxMoveCoefKnots, New_MDCurve_MoveCoef_len);

    if(pAmergeCtx->ConfigV1.MaxEnvLvKnots != New_OECurve_EnvLv_len) {
        free(pAmergeCtx->ConfigV1.EnvLv);
        free(pAmergeCtx->ConfigV1.OECurve_smooth);
        free(pAmergeCtx->ConfigV1.OECurve_offset);
        pAmergeCtx->ConfigV1.MaxEnvLvKnots = New_OECurve_EnvLv_len;
        pAmergeCtx->ConfigV1.EnvLv = (float*)malloc(sizeof(float) * New_OECurve_EnvLv_len);
        pAmergeCtx->ConfigV1.OECurve_smooth = (float*)malloc(sizeof(float) * New_OECurve_EnvLv_len);
        pAmergeCtx->ConfigV1.OECurve_offset = (float*)malloc(sizeof(float) * New_OECurve_EnvLv_len);
    }
    if(pAmergeCtx->ConfigV1.MaxMoveCoefKnots != New_MDCurve_MoveCoef_len) {
        free(pAmergeCtx->ConfigV1.MoveCoef);
        free(pAmergeCtx->ConfigV1.MDCurveLM_smooth);
        free(pAmergeCtx->ConfigV1.MDCurveLM_offset);
        free(pAmergeCtx->ConfigV1.MDCurveMS_smooth);
        free(pAmergeCtx->ConfigV1.MDCurveMS_offset);
        pAmergeCtx->ConfigV1.MaxMoveCoefKnots = New_MDCurve_MoveCoef_len;
        pAmergeCtx->ConfigV1.MoveCoef = (float*)malloc(sizeof(float) * New_MDCurve_MoveCoef_len);
        pAmergeCtx->ConfigV1.MDCurveLM_smooth = (float*)malloc(sizeof(float) * New_MDCurve_MoveCoef_len);
        pAmergeCtx->ConfigV1.MDCurveLM_offset = (float*)malloc(sizeof(float) * New_MDCurve_MoveCoef_len);
        pAmergeCtx->ConfigV1.MDCurveMS_smooth = (float*)malloc(sizeof(float) * New_MDCurve_MoveCoef_len);
        pAmergeCtx->ConfigV1.MDCurveMS_offset = (float*)malloc(sizeof(float) * New_MDCurve_MoveCoef_len);
    }

    AmergePrepareJsonUpdateConfig(pAmergeCtx);
...
(
    ...
    pAmergeCtx->ConfigV1.BaseFrm = BASEFRAME_LONG;
    pAmergeCtx->ConfigV1.ByPassThr = DAMPMIN;
    pAmergeCtx->ConfigV1.OECurve_damp = DAMPMIN;
    pAmergeCtx->ConfigV1.MDCurveLM_damp = DAMPMIN;
    pAmergeCtx->ConfigV1.MDCurveMS_damp = DAMPMIN;
    for (int i = 0; i < pAmergeCtx->ConfigV1.MaxEnvLvKnots; i++ ) {
        pAmergeCtx->ConfigV1.EnvLv[i] = ENVLVMIN;
        pAmergeCtx->ConfigV1.OECurve_smooth[i] = LIMIT_VALUE(pAmergeCtx->mergeAttr.attrV21.stManual.OECurve.Smooth, IQPARAMAX, IQPARAMIN);
        pAmergeCtx->ConfigV1.OECurve_offset[i] = LIMIT_VALUE(pAmergeCtx->mergeAttr.attrV21.stManual.OECurve.Offset, OECURVEOFFSETMAX, OECURVEOFFSETMIN);
    }
    for (int i = 0; i < pAmergeCtx->ConfigV1.MaxMoveCoefKnots; i++ ) {
        pAmergeCtx->ConfigV1.MoveCoef[i] = MOVECOEFMIN;
        pAmergeCtx->ConfigV1.MDCurveLM_smooth[i] = LIMIT_VALUE(pAmergeCtx->mergeAttr.attrV21.stManual.MDCurve.LM_smooth, IQPARAMAX, IQPARAMIN);
        pAmergeCtx->ConfigV1.MDCurveLM_offset[i] = LIMIT_VALUE(pAmergeCtx->mergeAttr.attrV21.stManual.MDCurve.LM_offset, IQPARAMAX, IQPARAMIN);
        pAmergeCtx->ConfigV1.MDCurveMS_smooth[i] = LIMIT_VALUE(pAmergeCtx->mergeAttr.attrV21.stManual.MDCurve.MS_smooth, IQPARAMAX, IQPARAMIN);
        pAmergeCtx->ConfigV1.MDCurveMS_offset[i] = LIMIT_VALUE(pAmergeCtx->mergeAttr.attrV21.stManual.MDCurve.MS_offset, IQPARAMAX, IQPARAMIN);
    }
...
(
    ...
    // initial checks
    DCT_ASSERT(pAmergeCtx->ConfigV1 != NULL);
...
)
{
    ...
    if(pAmergeCtx->frameCnt <= 2) //start frame
        ...
        ByPassThr = pAmergeCtx->ConfigV1.ByPassThr;
...
(
    ...
    *ppAmergeCtx = pAmergeCtx;
    ...
    memcpy(&pAmergeCtx->CalibDBV1, calibv2_amerge_calib, sizeof(CalibDbV2_merge_t));//load iq paras

    //malloc
    pAmergeCtx->ConfigV1.MaxEnvLvKnots = calibv2_amerge_calib->MergeTuningPara.OECurve.EnvLv_len;
    pAmergeCtx->ConfigV1.EnvLv = (float*)malloc(sizeof(float) * (calibv2_amerge_calib->MergeTuningPara.OECurve.EnvLv_len));
    pAmergeCtx->ConfigV1.OECurve_smooth = (float*)malloc(sizeof(float) * (calibv2_amerge_calib->MergeTuningPara.OECurve.EnvLv_len));
    pAmergeCtx->ConfigV1.OECurve_offset = (float*)malloc(sizeof(float) * (calibv2_amerge_calib->MergeTuningPara.OECurve.EnvLv_len));
    pAmergeCtx->ConfigV1.MaxMoveCoefKnots = calibv2_amerge_calib->MergeTuningPara.MDCurve.MoveCoef_len;
    pAmergeCtx->ConfigV1.MoveCoef = (float*)malloc(sizeof(float) * (calibv2_amerge_calib->MergeTuningPara.MDCurve.MoveCoef_len));
    pAmergeCtx->ConfigV1.MDCurveLM_smooth = (float*)malloc(sizeof(float) * (calibv2_amerge_calib->MergeTuningPara.MDCurve.MoveCoef_len));
    pAmergeCtx->ConfigV1.MDCurveLM_offset = (float*)malloc(sizeof(float) * (calibv2_amerge_calib->MergeTuningPara.MDCurve.MoveCoef_len));
    pAmergeCtx->ConfigV1.MDCurveMS_smooth = (float*)malloc(sizeof(float) * (calibv2_amerge_calib->MergeTuningPara.MDCurve.MoveCoef_len));
    pAmergeCtx->ConfigV1.MDCurveMS_offset = (float*)malloc(sizeof(float) * (calibv2_amerge_calib->MergeTuningPara.MDCurve.MoveCoef_len));
