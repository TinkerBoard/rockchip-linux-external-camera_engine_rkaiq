...
/******************************************************************************
 * AdrcProcUpdateConfigV2()
 *transfer html parameter into handle
 ***************************************************************************/
void AdrcProcUpdateConfigV2
(
    AdrcContext_t* pAdrcCtx
) {
    LOG1_ATMO( "%s:enter!\n", __FUNCTION__);
    ...
    if(pAdrcCtx->drcAttr.opMode > DRC_OPMODE_MANU)  {
        ...
        if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_DRC_GAIN) {
            pAdrcCtx->ConfigV2.DrcGain.EnvLv[0] = ADRCNORMALIZEMIN;
            pAdrcCtx->ConfigV2.DrcGain.DrcGain[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stDrcGain.DrcGain, DRCGAINMAX, DRCGAINMIN);
            pAdrcCtx->ConfigV2.DrcGain.Alpha[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stDrcGain.Alpha, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
            pAdrcCtx->ConfigV2.DrcGain.Clip[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stDrcGain.Clip, CLIPMAX, CLIPMIN);
        }
        else
            for (int i = 0; i < pAdrcCtx->ConfigV2.DrcGain.len; i++ ) {
                pAdrcCtx->ConfigV2.DrcGain.EnvLv[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.DrcGain.EnvLv[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
                pAdrcCtx->ConfigV2.DrcGain.DrcGain[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.DrcGain.DrcGain[i], DRCGAINMAX, DRCGAINMIN);
                pAdrcCtx->ConfigV2.DrcGain.Alpha[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.DrcGain.Alpha[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
                pAdrcCtx->ConfigV2.DrcGain.Clip[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.DrcGain.Clip[i], CLIPMAX, CLIPMIN);
            }
        //hight light
        if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_DRC_GAIN) {
            pAdrcCtx->ConfigV2.HiLit.EnvLv[0] = ADRCNORMALIZEMIN;
            pAdrcCtx->ConfigV2.HiLit.Strength[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stHiLit.Strength, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        }
        else
            for (int i = 0; i < pAdrcCtx->ConfigV2.HiLit.len; i++ ) {
                pAdrcCtx->ConfigV2.HiLit.EnvLv[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.HiLight.EnvLv[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
                pAdrcCtx->ConfigV2.HiLit.Strength[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.HiLight.Strength[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
            }
        // local
        if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_DRC_GAIN) {
            pAdrcCtx->ConfigV2.Local.EnvLv[0] = ADRCNORMALIZEMIN;
            pAdrcCtx->ConfigV2.Local.LocalWeit[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stLocalDataV30.LocalWeit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
            pAdrcCtx->ConfigV2.Local.LocalAutoEnable[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stLocalDataV30.LocalAutoEnable, ADRCNORMALIZEINTMAX, ADRCNORMALIZEINTMIN);
            pAdrcCtx->ConfigV2.Local.LocalAutoWeit[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stLocalDataV30.LocalAutoWeit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
            pAdrcCtx->ConfigV2.Local.GlobalContrast[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stLocalDataV30.GlobalContrast, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
            pAdrcCtx->ConfigV2.Local.LoLitContrast[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stLocalDataV30.LoLitContrast, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        }
        else
            for (int i = 0; i < pAdrcCtx->ConfigV2.Local.len; i++ ) {
                pAdrcCtx->ConfigV2.Local.EnvLv[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.LocalData.EnvLv[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
                pAdrcCtx->ConfigV2.Local.LocalWeit[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.LocalData.LocalWeit[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
                pAdrcCtx->ConfigV2.Local.LocalAutoEnable[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.LocalData.LocalAutoEnable[i], ADRCNORMALIZEINTMAX, ADRCNORMALIZEINTMIN);
                pAdrcCtx->ConfigV2.Local.LocalAutoWeit[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.LocalData.LocalAutoWeit[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
                pAdrcCtx->ConfigV2.Local.GlobalContrast[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.LocalData.GlobalContrast[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
                pAdrcCtx->ConfigV2.Local.LoLitContrast[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.LocalData.LoLitContrast[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
            }
        //others
        pAdrcCtx->ConfigV2.OutPutLongFrame = pAdrcCtx->CalibDBV2.DrcTuningPara.OutPutLongFrame;
        pAdrcCtx->ConfigV2.Others.curPixWeit = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.curPixWeit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Others.preFrameWeit = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.preFrameWeit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Others.Range_force_sgm = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.Range_force_sgm, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Others.Range_sgm_cur = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.Range_sgm_cur, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Others.Range_sgm_pre = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.Range_sgm_pre, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Others.Space_sgm_cur = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.Space_sgm_cur, SPACESGMMAX, SPACESGMMIN);
        pAdrcCtx->ConfigV2.Others.Space_sgm_pre = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.Space_sgm_pre, SPACESGMMAX, SPACESGMMIN);
        //compress and scale y
        pAdrcCtx->ConfigV2.Compress.Mode = pAdrcCtx->CalibDBV2.DrcTuningPara.CompressSetting.Mode;
        for(int i = 0; i < ADRC_Y_NUM; i++ ) {
            pAdrcCtx->ConfigV2.Others.Scale_y[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.Scale_y[i], SCALEYMAX, SCALEYMIN);
            pAdrcCtx->ConfigV2.Compress.Manual_curve[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.CompressSetting.Manual_curve[i], MANUALCURVEMAX, MANUALCURVEMIN);
        }
        pAdrcCtx->ConfigV2.Others.ByPassThr = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.ByPassThr, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Others.Edge_Weit = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.Edge_Weit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Others.IIR_frame = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.IIR_frame, IIRFRAMEMAX, IIRFRAMEMIN);
        pAdrcCtx->ConfigV2.Others.Tolerance = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.Tolerance, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Others.damp = LIMIT_VALUE(pAdrcCtx->CalibDBV2.DrcTuningPara.damp, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
    }
    else if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_MANU) {
        //drc gain
        pAdrcCtx->ConfigV2.DrcGain.EnvLv[0] = ADRCNORMALIZEMIN;
        pAdrcCtx->ConfigV2.DrcGain.DrcGain[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.DrcGain.DrcGain, DRCGAINMAX, DRCGAINMIN);
        pAdrcCtx->ConfigV2.DrcGain.Alpha[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.DrcGain.Alpha, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.DrcGain.Clip[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.DrcGain.Clip, CLIPMAX, CLIPMIN);
        //hight light
        pAdrcCtx->ConfigV2.HiLit.EnvLv[0] = ADRCNORMALIZEMIN;
        pAdrcCtx->ConfigV2.HiLit.Strength[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.HiLight.Strength, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        // local
        pAdrcCtx->ConfigV2.Local.EnvLv[0] = ADRCNORMALIZEMIN;
        pAdrcCtx->ConfigV2.Local.LocalWeit[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.LocalSetting.LocalData.LocalWeit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Local.LocalAutoEnable[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.LocalSetting.LocalData.LocalAutoEnable, ADRCNORMALIZEINTMAX, ADRCNORMALIZEINTMIN);
        pAdrcCtx->ConfigV2.Local.LocalAutoWeit[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.LocalSetting.LocalData.LocalAutoWeit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Local.GlobalContrast[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.LocalSetting.LocalData.GlobalContrast, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Local.LoLitContrast[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.LocalSetting.LocalData.LoLitContrast, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        //others
        pAdrcCtx->ConfigV2.OutPutLongFrame = pAdrcCtx->drcAttr.stManualV30.OutPutLongFrame;
        pAdrcCtx->ConfigV2.Others.curPixWeit = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.LocalSetting.curPixWeit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Others.preFrameWeit = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.LocalSetting.preFrameWeit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Others.Range_force_sgm = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.LocalSetting.Range_force_sgm, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Others.Range_sgm_cur = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.LocalSetting.Range_sgm_cur, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Others.Range_sgm_pre = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.LocalSetting.Range_sgm_pre, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Others.Space_sgm_cur = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.LocalSetting.Space_sgm_cur, SPACESGMMAX, SPACESGMMIN);
        pAdrcCtx->ConfigV2.Others.Space_sgm_pre = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.LocalSetting.Space_sgm_pre, SPACESGMMAX, SPACESGMMIN);
        for(int i = 0; i < ADRC_Y_NUM; i++ )
            pAdrcCtx->ConfigV2.Others.Scale_y[i] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.Scale_y[i], SCALEYMAX, SCALEYMIN);
        pAdrcCtx->ConfigV2.Others.ByPassThr = ADRCNORMALIZEMIN;
        pAdrcCtx->ConfigV2.Others.Edge_Weit = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.Edge_Weit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV2.Others.IIR_frame = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV30.IIR_frame, IIRFRAMEMAX, IIRFRAMEMIN);
        pAdrcCtx->ConfigV2.Others.Tolerance = ADRCNORMALIZEMIN;
        pAdrcCtx->ConfigV2.Others.damp = ADRCNORMALIZEMIN;
    }
...
/******************************************************************************
 * DrcProcApiMallocAndUpdateConfig()
 ***************************************************************************/
void DrcProcApiMallocAndUpdateConfig
(
    AdrcContext_t* pAdrcCtx
) {
    LOG1_ATMO( "%s:enter!\n", __FUNCTION__);
    ...
    if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_MANU) {
        ...
        New_DrcGain_EnvLv_len = 1;
        New_HiLit_EnvLv_len = pAdrcCtx->CalibDBV2.DrcTuningPara.HiLight.EnvLv_len;
        New_Local_EnvLv_len = pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.LocalData.EnvLv_len;
    }
    else if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_HILIT) {
        New_DrcGain_EnvLv_len = pAdrcCtx->CalibDBV2.DrcTuningPara.DrcGain.EnvLv_len;
        New_HiLit_EnvLv_len = 1;
        New_Local_EnvLv_len = pAdrcCtx->CalibDBV2.DrcTuningPara.LocalSetting.LocalData.EnvLv_len;
    }
    ...
    LOG1_ATMO( "%s: Pre DrcGain len:%d, Cur DrcGain len:%d\n", __FUNCTION__, pAdrcCtx->ConfigV2.DrcGain.len, New_DrcGain_EnvLv_len);
    LOG1_ATMO( "%s: Pre HiLit len:%d, Cur HiLit len:%d\n", __FUNCTION__, pAdrcCtx->ConfigV2.HiLit.len, New_HiLit_EnvLv_len);
    LOG1_ATMO( "%s: Pre Local len:%d, Cur Local len:%d\n", __FUNCTION__, pAdrcCtx->ConfigV2.Local.len, New_Local_EnvLv_len);

    //drc gain
    if(pAdrcCtx->ConfigV2.DrcGain.len != New_DrcGain_EnvLv_len) {
        free(pAdrcCtx->ConfigV2.DrcGain.EnvLv);
        free(pAdrcCtx->ConfigV2.DrcGain.DrcGain);
        free(pAdrcCtx->ConfigV2.DrcGain.Alpha);
        free(pAdrcCtx->ConfigV2.DrcGain.Clip);
        pAdrcCtx->ConfigV2.DrcGain.len = New_DrcGain_EnvLv_len;
        pAdrcCtx->ConfigV2.DrcGain.EnvLv = (float*)malloc(sizeof(float) * (New_DrcGain_EnvLv_len));
        pAdrcCtx->ConfigV2.DrcGain.DrcGain = (float*)malloc(sizeof(float) * (New_DrcGain_EnvLv_len));
        pAdrcCtx->ConfigV2.DrcGain.Alpha = (float*)malloc(sizeof(float) * (New_DrcGain_EnvLv_len));
        pAdrcCtx->ConfigV2.DrcGain.Clip = (float*)malloc(sizeof(float) * (New_DrcGain_EnvLv_len));
    }

    //hilit
    if(pAdrcCtx->ConfigV2.HiLit.len != New_HiLit_EnvLv_len) {
        free(pAdrcCtx->ConfigV2.HiLit.EnvLv);
        free(pAdrcCtx->ConfigV2.HiLit.Strength);
        pAdrcCtx->ConfigV2.HiLit.len = New_HiLit_EnvLv_len;
        pAdrcCtx->ConfigV2.HiLit.EnvLv = (float*)malloc(sizeof(float) * (New_HiLit_EnvLv_len));
        pAdrcCtx->ConfigV2.HiLit.Strength = (float*)malloc(sizeof(float) * (New_HiLit_EnvLv_len));
    }

    //local
    if(pAdrcCtx->ConfigV2.Local.len != New_Local_EnvLv_len) {
        free(pAdrcCtx->ConfigV2.Local.EnvLv);
        free(pAdrcCtx->ConfigV2.Local.LocalWeit);
        free(pAdrcCtx->ConfigV2.Local.LocalAutoEnable);
        free(pAdrcCtx->ConfigV2.Local.LocalAutoWeit);
        free(pAdrcCtx->ConfigV2.Local.GlobalContrast);
        free(pAdrcCtx->ConfigV2.Local.LoLitContrast);
        pAdrcCtx->ConfigV2.Local.len = New_Local_EnvLv_len;
        pAdrcCtx->ConfigV2.Local.EnvLv = (float*)malloc(sizeof(float) * (New_Local_EnvLv_len));
        pAdrcCtx->ConfigV2.Local.LocalWeit = (float*)malloc(sizeof(float) * (New_Local_EnvLv_len));
        pAdrcCtx->ConfigV2.Local.LocalAutoEnable = (int*)malloc(sizeof(float) * (New_Local_EnvLv_len));
        pAdrcCtx->ConfigV2.Local.LocalAutoWeit = (float*)malloc(sizeof(float) * (New_Local_EnvLv_len));
        pAdrcCtx->ConfigV2.Local.GlobalContrast = (float*)malloc(sizeof(float) * (New_Local_EnvLv_len));
        pAdrcCtx->ConfigV2.Local.LoLitContrast = (float*)malloc(sizeof(float) * (New_Local_EnvLv_len));
    }

    AdrcProcUpdateConfigV2(pAdrcCtx);

    LOG1_ATMO( "%s:exit!\n", __FUNCTION__);
}
/******************************************************************************
 * AdrcPrePareJsonUpdateConfigV2()
 *transfer html parameter into handle
 ***************************************************************************/
void AdrcPrePareJsonUpdateConfigV2
(
    AdrcContext_t* pAdrcCtx
) {
    LOG1_ATMO( "%s:enter!\n", __FUNCTION__);
...
(
    ...
    AdrcPrePareJsonUpdateConfigV2(pAdrcCtx);
...
)
{
    ...
    //store enable
    pAdrcCtx->CurrData.Enable = pAdrcCtx->ConfigV2.Enable;

    //get Drc gain
    pAdrcCtx->CurrData.HandleData.Drc_v30.DrcGain = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV2.DrcGain.EnvLv, pAdrcCtx->ConfigV2.DrcGain.DrcGain, pAdrcCtx->ConfigV2.DrcGain.len);
    pAdrcCtx->CurrData.HandleData.Drc_v30.Alpha = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV2.DrcGain.EnvLv, pAdrcCtx->ConfigV2.DrcGain.Alpha, pAdrcCtx->ConfigV2.DrcGain.len);
    pAdrcCtx->CurrData.HandleData.Drc_v30.Clip = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV2.DrcGain.EnvLv, pAdrcCtx->ConfigV2.DrcGain.Clip, pAdrcCtx->ConfigV2.DrcGain.len);

    //get hi lit
    pAdrcCtx->CurrData.HandleData.Drc_v30.Strength = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV2.HiLit.EnvLv, pAdrcCtx->ConfigV2.HiLit.Strength, pAdrcCtx->ConfigV2.HiLit.len);

    //get local
    pAdrcCtx->CurrData.HandleData.Drc_v30.LocalWeit = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV2.Local.EnvLv, pAdrcCtx->ConfigV2.Local.LocalWeit, pAdrcCtx->ConfigV2.Local.len);
    pAdrcCtx->CurrData.HandleData.Drc_v30.GlobalContrast = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV2.Local.EnvLv, pAdrcCtx->ConfigV2.Local.GlobalContrast, pAdrcCtx->ConfigV2.Local.len);
    pAdrcCtx->CurrData.HandleData.Drc_v30.LoLitContrast = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV2.Local.EnvLv, pAdrcCtx->ConfigV2.Local.LoLitContrast, pAdrcCtx->ConfigV2.Local.len);
    pAdrcCtx->CurrData.HandleData.Drc_v30.LocalAutoEnable = DrcGetCurrParaInt(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV2.Local.EnvLv, pAdrcCtx->ConfigV2.Local.LocalAutoEnable, pAdrcCtx->ConfigV2.Local.len);
    pAdrcCtx->CurrData.HandleData.Drc_v30.LocalAutoWeit = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV2.Local.EnvLv, pAdrcCtx->ConfigV2.Local.LocalAutoWeit, pAdrcCtx->ConfigV2.Local.len);

    //compress
    pAdrcCtx->CurrData.HandleData.Drc_v30.Mode = pAdrcCtx->ConfigV2.Compress.Mode;
    for(int i = 0; i < ADRC_Y_NUM; i++)
        pAdrcCtx->CurrData.HandleData.Drc_v30.Manual_curve[i] = pAdrcCtx->ConfigV2.Compress.Manual_curve[i];

    //others
    if(0 != memcmp(&pAdrcCtx->CurrData.Others, &pAdrcCtx->ConfigV2.Others, sizeof(DrcOhters_t)))
        memcpy(&pAdrcCtx->CurrData.Others, &pAdrcCtx->ConfigV2.Others, sizeof(DrcOhters_t));//load iq paras
...
)
{
    ...
    if(pAdrcCtx->FrameNumber == HDR_2X_NUM || pAdrcCtx->FrameNumber == HDR_3X_NUM)
        pAdrcCtx->ConfigV2.Enable = true;
    else if(pAdrcCtx->FrameNumber == LINEAR_NUM) {
        if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_API_OFF)
            pAdrcCtx->ConfigV2.Enable = pAdrcCtx->CalibDBV2.DrcTuningPara.Enable;
        else if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_MANU)
            pAdrcCtx->ConfigV2.Enable = pAdrcCtx->drcAttr.stManualV30.Enable;
            ...
            pAdrcCtx->ConfigV2.Enable = false;
        }
    }
...
)
{
    ...
    if(pAdrcCtx->ConfigV2.OutPutLongFrame)
...
)
{
    ...
    ByPassThr = pAdrcCtx->ConfigV2.Others.ByPassThr;
...
(
    ...
    *ppAdrcCtx = pAdrcCtx;
    ...
    //malloc DrcGain
    pAdrcCtx->ConfigV2.DrcGain.len = calibv2_adrc_calib->DrcTuningPara.DrcGain.EnvLv_len;
    pAdrcCtx->ConfigV2.DrcGain.EnvLv = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.DrcGain.EnvLv_len));
    pAdrcCtx->ConfigV2.DrcGain.DrcGain = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.DrcGain.EnvLv_len));
    pAdrcCtx->ConfigV2.DrcGain.Alpha = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.DrcGain.EnvLv_len));
    pAdrcCtx->ConfigV2.DrcGain.Clip = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.DrcGain.EnvLv_len));

    //malloc HiLit
    pAdrcCtx->ConfigV2.HiLit.len = calibv2_adrc_calib->DrcTuningPara.HiLight.EnvLv_len;
    pAdrcCtx->ConfigV2.HiLit.EnvLv = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.HiLight.EnvLv_len));
    pAdrcCtx->ConfigV2.HiLit.Strength = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.HiLight.EnvLv_len));

    //malloc local
    pAdrcCtx->ConfigV2.Local.len = calibv2_adrc_calib->DrcTuningPara.LocalSetting.LocalData.EnvLv_len;
    pAdrcCtx->ConfigV2.Local.EnvLv = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.LocalSetting.LocalData.EnvLv_len));
    pAdrcCtx->ConfigV2.Local.LocalAutoEnable = (int*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.LocalSetting.LocalData.EnvLv_len));
    pAdrcCtx->ConfigV2.Local.LocalAutoWeit = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.LocalSetting.LocalData.EnvLv_len));
    pAdrcCtx->ConfigV2.Local.LocalWeit = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.LocalSetting.LocalData.EnvLv_len));
    pAdrcCtx->ConfigV2.Local.GlobalContrast = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.LocalSetting.LocalData.EnvLv_len));
    pAdrcCtx->ConfigV2.Local.LoLitContrast = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.LocalSetting.LocalData.EnvLv_len));

    pAdrcCtx->drcAttr.Version = ADRC_VERSION_3588;
    pAdrcCtx->drcAttr.opMode = DRC_OPMODE_API_OFF;
    ConfigV30(pAdrcCtx); //set default para
    memcpy(&pAdrcCtx->CalibDBV2, calibv2_adrc_calib, sizeof(CalibDbV2_drc_V2_t));//load iq paras
