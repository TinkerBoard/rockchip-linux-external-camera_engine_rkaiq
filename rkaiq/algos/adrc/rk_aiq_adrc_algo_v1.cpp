...
/******************************************************************************
 * AdrcProcUpdateConfigV1()
 *transfer html parameter into handle
 ***************************************************************************/
void AdrcProcUpdateConfigV1
(
    AdrcContext_t* pAdrcCtx
) {
    LOG1_ATMO( "%s:enter!\n", __FUNCTION__);
    ...
    if (pAdrcCtx->drcAttr.opMode > DRC_OPMODE_MANU) {
        ...
        if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_DRC_GAIN) {
            pAdrcCtx->ConfigV1.DrcGain.EnvLv[0] = ADRCNORMALIZEMIN;
            pAdrcCtx->ConfigV1.DrcGain.DrcGain[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stDrcGain.DrcGain, DRCGAINMAX, DRCGAINMIN);
            pAdrcCtx->ConfigV1.DrcGain.Alpha[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stDrcGain.Alpha, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
            pAdrcCtx->ConfigV1.DrcGain.Clip[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stDrcGain.Clip, CLIPMAX, CLIPMIN);
        }
        else
            for (int i = 0; i < pAdrcCtx->ConfigV1.DrcGain.len; i++ ) {
                pAdrcCtx->ConfigV1.DrcGain.EnvLv[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.DrcGain.EnvLv[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
                pAdrcCtx->ConfigV1.DrcGain.DrcGain[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.DrcGain.DrcGain[i], DRCGAINMAX, DRCGAINMIN);
                pAdrcCtx->ConfigV1.DrcGain.Alpha[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.DrcGain.Alpha[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
                pAdrcCtx->ConfigV1.DrcGain.Clip[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.DrcGain.Clip[i], CLIPMAX, CLIPMIN);
            }
        //hight ligjt
        if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_HILIT) {
            pAdrcCtx->ConfigV1.HiLit.EnvLv[0] = ADRCNORMALIZEMIN;
            pAdrcCtx->ConfigV1.HiLit.Strength[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stHiLit.Strength, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        }
        else
            for (int i = 0; i < pAdrcCtx->ConfigV1.HiLit.len; i++ ) {
                pAdrcCtx->ConfigV1.HiLit.EnvLv[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.HiLight.EnvLv[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
                pAdrcCtx->ConfigV1.HiLit.Strength[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.HiLight.Strength[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
            }
        // local
        if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_LOCAL_TMO) {
            pAdrcCtx->ConfigV1.Local.EnvLv[0] = ADRCNORMALIZEMIN;
            pAdrcCtx->ConfigV1.Local.LocalWeit[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stLocalDataV21.LocalWeit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
            pAdrcCtx->ConfigV1.Local.GlobalContrast[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stLocalDataV21.GlobalContrast, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
            pAdrcCtx->ConfigV1.Local.LoLitContrast[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stLocalDataV21.LoLitContrast, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        }
        else
            for (int i = 0; i < pAdrcCtx->ConfigV1.Local.len; i++ ) {
                pAdrcCtx->ConfigV1.Local.EnvLv[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.LocalTMOSetting.LocalTMOData.EnvLv[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
                pAdrcCtx->ConfigV1.Local.LocalWeit[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.LocalTMOSetting.LocalTMOData.LocalWeit[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
                pAdrcCtx->ConfigV1.Local.GlobalContrast[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.LocalTMOSetting.LocalTMOData.GlobalContrast[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
                pAdrcCtx->ConfigV1.Local.LoLitContrast[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.LocalTMOSetting.LocalTMOData.LoLitContrast[i], ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
            }
        //others
        pAdrcCtx->ConfigV1.OutPutLongFrame = pAdrcCtx->CalibDBV1.DrcTuningPara.OutPutLongFrame;
        pAdrcCtx->ConfigV1.Others.curPixWeit = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.LocalTMOSetting.curPixWeit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Others.preFrameWeit = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.LocalTMOSetting.preFrameWeit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Others.Range_force_sgm = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.LocalTMOSetting.Range_force_sgm, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Others.Range_sgm_cur = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.LocalTMOSetting.Range_sgm_cur, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Others.Range_sgm_pre = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.LocalTMOSetting.Range_sgm_pre, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Others.Space_sgm_cur = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.LocalTMOSetting.Space_sgm_cur, SPACESGMMAX, SPACESGMMIN);
        pAdrcCtx->ConfigV1.Others.Space_sgm_pre = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.LocalTMOSetting.Space_sgm_pre, SPACESGMMAX, SPACESGMMIN);
        //compress and scale y
        pAdrcCtx->ConfigV1.Compress.Mode = pAdrcCtx->CalibDBV1.DrcTuningPara.CompressSetting.Mode;
        for(int i = 0; i < ADRC_Y_NUM; i++ ) {
            pAdrcCtx->ConfigV1.Others.Scale_y[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.Scale_y[i], SCALEYMAX, SCALEYMIN);
            pAdrcCtx->ConfigV1.Compress.Manual_curve[i] = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.CompressSetting.Manual_curve[i], MANUALCURVEMAX, MANUALCURVEMIN);
        }
        pAdrcCtx->ConfigV1.Others.ByPassThr = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.ByPassThr, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Others.Edge_Weit = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.Edge_Weit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Others.IIR_frame = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.IIR_frame, IIRFRAMEMAX, IIRFRAMEMIN);
        pAdrcCtx->ConfigV1.Others.Tolerance = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.Tolerance, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Others.damp = LIMIT_VALUE(pAdrcCtx->CalibDBV1.DrcTuningPara.damp, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
    }
    else if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_MANU) {
        //drc gain
        pAdrcCtx->ConfigV1.DrcGain.EnvLv[0] = ADRCNORMALIZEMIN;
        pAdrcCtx->ConfigV1.DrcGain.DrcGain[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.DrcGain.DrcGain, DRCGAINMAX, DRCGAINMIN);
        pAdrcCtx->ConfigV1.DrcGain.Alpha[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.DrcGain.Alpha, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.DrcGain.Clip[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.DrcGain.Clip, CLIPMAX, CLIPMIN);
        //hight ligjt
        pAdrcCtx->ConfigV1.HiLit.EnvLv[0] = ADRCNORMALIZEMIN;
        pAdrcCtx->ConfigV1.HiLit.Strength[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.HiLit.Strength, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        // local
        pAdrcCtx->ConfigV1.Local.EnvLv[0] = ADRCNORMALIZEMIN;
        pAdrcCtx->ConfigV1.Local.LocalWeit[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.LocalSetting.LocalData.LocalWeit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Local.GlobalContrast[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.LocalSetting.LocalData.GlobalContrast, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Local.LoLitContrast[0] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.LocalSetting.LocalData.LoLitContrast, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        //others
        pAdrcCtx->ConfigV1.OutPutLongFrame = pAdrcCtx->drcAttr.stManualV21.OutPutLongFrame;
        pAdrcCtx->ConfigV1.Others.curPixWeit = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.LocalSetting.curPixWeit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Others.preFrameWeit = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.LocalSetting.preFrameWeit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Others.Range_force_sgm = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.LocalSetting.Range_force_sgm, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Others.Range_sgm_cur = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.LocalSetting.Range_sgm_cur, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Others.Range_sgm_pre = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.LocalSetting.Range_sgm_pre, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Others.Space_sgm_cur = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.LocalSetting.Space_sgm_cur, SPACESGMMAX, SPACESGMMIN);
        pAdrcCtx->ConfigV1.Others.Space_sgm_pre = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.LocalSetting.Space_sgm_pre, SPACESGMMAX, SPACESGMMIN);
        for(int i = 0; i < ADRC_Y_NUM; i++ )
            pAdrcCtx->ConfigV1.Others.Scale_y[i] = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.Scale_y[i], SCALEYMAX, SCALEYMIN);
        pAdrcCtx->ConfigV1.Others.ByPassThr = ADRCNORMALIZEMIN;
        pAdrcCtx->ConfigV1.Others.Edge_Weit = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.Edge_Weit, ADRCNORMALIZEMAX, ADRCNORMALIZEMIN);
        pAdrcCtx->ConfigV1.Others.IIR_frame = LIMIT_VALUE(pAdrcCtx->drcAttr.stManualV21.IIR_frame, IIRFRAMEMAX, IIRFRAMEMIN);
        pAdrcCtx->ConfigV1.Others.Tolerance = ADRCNORMALIZEMIN;
        pAdrcCtx->ConfigV1.Others.damp = ADRCNORMALIZEMIN;
    }
...
(
    ...
    if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_MANU) {
        ...
        New_DrcGain_EnvLv_len = 1;
        New_HiLit_EnvLv_len = pAdrcCtx->CalibDBV1.DrcTuningPara.HiLight.EnvLv_len;
        New_Local_EnvLv_len = pAdrcCtx->CalibDBV1.DrcTuningPara.LocalTMOSetting.LocalTMOData.EnvLv_len;
    }
    else if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_HILIT) {
        New_DrcGain_EnvLv_len = pAdrcCtx->CalibDBV1.DrcTuningPara.DrcGain.EnvLv_len;
        New_HiLit_EnvLv_len = 1;
        New_Local_EnvLv_len = pAdrcCtx->CalibDBV1.DrcTuningPara.LocalTMOSetting.LocalTMOData.EnvLv_len;
    }
    ...
    LOG1_ATMO( "%s: Pre DrcGain len:%d, Cur DrcGain len:%d\n", __FUNCTION__, pAdrcCtx->ConfigV1.DrcGain.len, New_DrcGain_EnvLv_len);
    LOG1_ATMO( "%s: Pre HiLit len:%d, Cur HiLit len:%d\n", __FUNCTION__, pAdrcCtx->ConfigV1.HiLit.len, New_HiLit_EnvLv_len);
    LOG1_ATMO( "%s: Pre Local len:%d, Cur Local len:%d\n", __FUNCTION__, pAdrcCtx->ConfigV1.Local.len, New_Local_EnvLv_len);

    //drc gain
    if(pAdrcCtx->ConfigV1.DrcGain.len != New_DrcGain_EnvLv_len) {
        free(pAdrcCtx->ConfigV1.DrcGain.EnvLv);
        free(pAdrcCtx->ConfigV1.DrcGain.DrcGain);
        free(pAdrcCtx->ConfigV1.DrcGain.Alpha);
        free(pAdrcCtx->ConfigV1.DrcGain.Clip);
        pAdrcCtx->ConfigV1.DrcGain.len = New_DrcGain_EnvLv_len;
        pAdrcCtx->ConfigV1.DrcGain.EnvLv = (float*)malloc(sizeof(float) * (New_DrcGain_EnvLv_len));
        pAdrcCtx->ConfigV1.DrcGain.DrcGain = (float*)malloc(sizeof(float) * (New_DrcGain_EnvLv_len));
        pAdrcCtx->ConfigV1.DrcGain.Alpha = (float*)malloc(sizeof(float) * (New_DrcGain_EnvLv_len));
        pAdrcCtx->ConfigV1.DrcGain.Clip = (float*)malloc(sizeof(float) * (New_DrcGain_EnvLv_len));
    }

    //hilit
    if(pAdrcCtx->ConfigV1.HiLit.len != New_HiLit_EnvLv_len) {
        free(pAdrcCtx->ConfigV1.HiLit.EnvLv);
        free(pAdrcCtx->ConfigV1.HiLit.Strength);
        pAdrcCtx->ConfigV1.HiLit.len = New_HiLit_EnvLv_len;
        pAdrcCtx->ConfigV1.HiLit.EnvLv = (float*)malloc(sizeof(float) * (New_HiLit_EnvLv_len));
        pAdrcCtx->ConfigV1.HiLit.Strength = (float*)malloc(sizeof(float) * (New_HiLit_EnvLv_len));
    }

    //local
    if(pAdrcCtx->ConfigV1.Local.len != New_Local_EnvLv_len) {
        free(pAdrcCtx->ConfigV1.Local.EnvLv);
        free(pAdrcCtx->ConfigV1.Local.LocalWeit);
        free(pAdrcCtx->ConfigV1.Local.GlobalContrast);
        free(pAdrcCtx->ConfigV1.Local.LoLitContrast);
        pAdrcCtx->ConfigV1.Local.len = New_Local_EnvLv_len;
        pAdrcCtx->ConfigV1.Local.EnvLv = (float*)malloc(sizeof(float) * (New_Local_EnvLv_len));
        pAdrcCtx->ConfigV1.Local.LocalWeit = (float*)malloc(sizeof(float) * (New_Local_EnvLv_len));
        pAdrcCtx->ConfigV1.Local.GlobalContrast = (float*)malloc(sizeof(float) * (New_Local_EnvLv_len));
        pAdrcCtx->ConfigV1.Local.LoLitContrast = (float*)malloc(sizeof(float) * (New_Local_EnvLv_len));
    }

    AdrcProcUpdateConfigV1(pAdrcCtx);
 ...
 *transfer html parameter into handle
 ***************************************************************************/
void AdrcPrePareJsonUpdateConfigV1
(
    AdrcContext_t* pAdrcCtx
) {
    LOG1_ATMO( "%s:enter!\n", __FUNCTION__);
...
(
    ...
    AdrcPrePareJsonUpdateConfigV1(pAdrcCtx);
...
)
{
    ...
    //store enable
    pAdrcCtx->CurrData.Enable = pAdrcCtx->ConfigV1.Enable;

    //get Drc gain
    pAdrcCtx->CurrData.HandleData.Drc_v21.DrcGain = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV1.DrcGain.EnvLv, pAdrcCtx->ConfigV1.DrcGain.DrcGain, pAdrcCtx->ConfigV1.DrcGain.len);
    pAdrcCtx->CurrData.HandleData.Drc_v21.Alpha = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV1.DrcGain.EnvLv, pAdrcCtx->ConfigV1.DrcGain.Alpha, pAdrcCtx->ConfigV1.DrcGain.len);
    pAdrcCtx->CurrData.HandleData.Drc_v21.Clip = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV1.DrcGain.EnvLv, pAdrcCtx->ConfigV1.DrcGain.Clip, pAdrcCtx->ConfigV1.DrcGain.len);

    //get hi lit
    pAdrcCtx->CurrData.HandleData.Drc_v21.Strength = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV1.HiLit.EnvLv, pAdrcCtx->ConfigV1.HiLit.Strength, pAdrcCtx->ConfigV1.HiLit.len);

    //get local
    pAdrcCtx->CurrData.HandleData.Drc_v21.LocalWeit = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV1.Local.EnvLv, pAdrcCtx->ConfigV1.Local.LocalWeit, pAdrcCtx->ConfigV1.Local.len);
    pAdrcCtx->CurrData.HandleData.Drc_v21.GlobalContrast = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV1.Local.EnvLv, pAdrcCtx->ConfigV1.Local.GlobalContrast, pAdrcCtx->ConfigV1.Local.len);
    pAdrcCtx->CurrData.HandleData.Drc_v21.LoLitContrast = DrcGetCurrPara(pAdrcCtx->CurrData.EnvLv,
            pAdrcCtx->ConfigV1.Local.EnvLv, pAdrcCtx->ConfigV1.Local.LoLitContrast, pAdrcCtx->ConfigV1.Local.len);

    //compress
    pAdrcCtx->CurrData.HandleData.Drc_v21.Mode = pAdrcCtx->ConfigV1.Compress.Mode;
    for(int i = 0; i < ADRC_Y_NUM; i++)
        pAdrcCtx->CurrData.HandleData.Drc_v21.Manual_curve[i] = pAdrcCtx->ConfigV1.Compress.Manual_curve[i];

    //others
    if(0 != memcmp(&pAdrcCtx->CurrData.Others, &pAdrcCtx->ConfigV1.Others, sizeof(DrcOhters_t)))
        memcpy(&pAdrcCtx->CurrData.Others, &pAdrcCtx->ConfigV1.Others, sizeof(DrcOhters_t));//load iq paras
...
)
{
    ...
    if(pAdrcCtx->FrameNumber == HDR_2X_NUM)
        pAdrcCtx->ConfigV1.Enable = true;
    else if(pAdrcCtx->FrameNumber == LINEAR_NUM) {
        if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_API_OFF)
            pAdrcCtx->ConfigV1.Enable = pAdrcCtx->CalibDBV1.DrcTuningPara.Enable;
        else if(pAdrcCtx->drcAttr.opMode == DRC_OPMODE_MANU)
            pAdrcCtx->ConfigV1.Enable = pAdrcCtx->drcAttr.stManualV21.Enable;
            ...
            pAdrcCtx->ConfigV1.Enable = false;
        }
    }
...
)
{
    ...
    if(pAdrcCtx->ConfigV1.OutPutLongFrame)
...
)
{
    ...
    ByPassThr = pAdrcCtx->ConfigV1.Others.ByPassThr;
...
(
    ...
    *ppAdrcCtx = pAdrcCtx;
    ...
    //malloc DrcGain
    pAdrcCtx->ConfigV1.DrcGain.len = calibv2_adrc_calib->DrcTuningPara.DrcGain.EnvLv_len;
    pAdrcCtx->ConfigV1.DrcGain.EnvLv = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.DrcGain.EnvLv_len));
    pAdrcCtx->ConfigV1.DrcGain.DrcGain = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.DrcGain.EnvLv_len));
    pAdrcCtx->ConfigV1.DrcGain.Alpha = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.DrcGain.EnvLv_len));
    pAdrcCtx->ConfigV1.DrcGain.Clip = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.DrcGain.EnvLv_len));

    //malloc HiLit
    pAdrcCtx->ConfigV1.HiLit.len = calibv2_adrc_calib->DrcTuningPara.HiLight.EnvLv_len;
    pAdrcCtx->ConfigV1.HiLit.EnvLv = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.HiLight.EnvLv_len));
    pAdrcCtx->ConfigV1.HiLit.Strength = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.HiLight.EnvLv_len));

    //malloc local
    pAdrcCtx->ConfigV1.Local.len = calibv2_adrc_calib->DrcTuningPara.LocalTMOSetting.LocalTMOData.EnvLv_len;
    pAdrcCtx->ConfigV1.Local.EnvLv = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.LocalTMOSetting.LocalTMOData.EnvLv_len));
    pAdrcCtx->ConfigV1.Local.LocalWeit = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.LocalTMOSetting.LocalTMOData.EnvLv_len));
    pAdrcCtx->ConfigV1.Local.GlobalContrast = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.LocalTMOSetting.LocalTMOData.EnvLv_len));
    pAdrcCtx->ConfigV1.Local.LoLitContrast = (float*)malloc(sizeof(float) * (calibv2_adrc_calib->DrcTuningPara.LocalTMOSetting.LocalTMOData.EnvLv_len));

    pAdrcCtx->drcAttr.Version = ADRC_VERSION_356X;
    pAdrcCtx->drcAttr.opMode = DRC_OPMODE_API_OFF;
    ConfigV21(pAdrcCtx); //set default para
    memcpy(&pAdrcCtx->CalibDBV1, calibv2_adrc_calib, sizeof(CalibDbV2_drc_t));//load iq paras
